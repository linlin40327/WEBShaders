...
void main() {
  vec3 result = vec3(1.0, 0.843, 0.0);
  vec3 light = vec3(.0);
  ...
  float phongValue = max(0.0,dot(reflectDir,viewDir));
  phongValue = pow(phongValue, specularPower);

  vec3 specular = phongValue * lightColor;

  light = ambient * ambientPower + hemi * hemispherePower + sunLight * sunPower;

  // result
  result = light * result + (specularButtom ? specular:vec3(.0));
